...
The browser client connects to `wss://66.135.12.137.sslip.io` by default. To run the matchmaker locally or on a VPS:
...
Override the browser WebSocket endpoint before loading the game with `window.NINE_LIVES_WS_URL = 'ws://host:8797'` or another secure `wss://` URL.
